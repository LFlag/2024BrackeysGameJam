...
// 输入管理器 提供外部检测按键按下、抬起功能
    public class InputMgr : BaseManager<InputMgr>
    {
        ...
        {
            if (Input.GetKey(key))
                EventCenter.Instance.Broadcast(new KeyStay() { Key = key });
        ...
        private void GetAxis(E_InputAxis axis)
        {
            ...
            switch (axis)
            {
                case E_InputAxis.Horizontal:
                    ...
                    // EventCenter.Instance.BroadcastEvent<E_InputAxis, float>(E_EventType.GetAxis, axis,
